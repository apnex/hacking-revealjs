...
function addPath(points, start = 0, end = 0) {
	...
	path.linewidth = scale.x / 2;
	path.fill = 'none';
	path.stroke = colours['mLight-Blue-600'];
	group1.add(path);
	if(start) {
		let circle = two.makeCircle(newPath[0].x, newPath[0].y, scale.x / 4);
	...
	if(end) {
		let circle = two.makeCircle(newPath[newPath.length - 1].x, newPath[newPath.length - 1].y, scale.x / 4);
		circle.fill = colours['mLight-Green-300'];
		circle.stroke = colours['mLight-Green-500'];
		circle.linewidth = scale.x / 4;
		group1.add(circle);
...
group2.rotation = rot3;
group2.translation.set(-100, 0);
main.add(group2)
